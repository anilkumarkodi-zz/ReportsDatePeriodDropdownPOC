    ...
    this.getEventVizObjects = function(user, dataSetCode, selectedTimePeriod){
        ...
        var getCharts = function(){
            var chartsSuccessPromise = function(response){
                eventVizObjects.push(response.data.eventCharts)
            };
        ...
        var getReportTables = function(){
            var reportTablesSuccessPromise = function(response){
                eventVizObjects.push(response.data.eventReports)
            };
        ...
        eventVizObjects.isResolved = Promise.all(promises);
        return Promise.all(promises)
            .then(function(){
                return _.flatten(eventVizObjects);
            })
    };
}]);
